...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Recebe qual ID foi enviado (Sessão ou Projeto direto)
    $id_sessao = !empty($_POST['sessao_id']) ? $_POST['sessao_id'] : null;
    $id_projeto = !empty($_POST['projeto_id']) ? $_POST['projeto_id'] : null;

    $perfil = $_SESSION['usuario_perfil'];

    // Fallback dinâmico para garantir que nunca fique numa tela branca
    ...
    // Se não enviou nenhum dos dois IDs, aborta e volta
    ...
    try {
        ...
        // Se der erro no banco, devolve com erro
        header("Location: ../pages/" . $pagina_origem . "?erro=bd");
        exit();
    }
} else {
    // Acesso direto pela URL sem botão, bloqueia e devolve
